...
ARQ_HISTORICO_TEMPERATURA = DISCO_PATH / "historico_temperatura.json"
...
def atualizar_canal_push_token(cliente_id: str, push_token: str, canal: str) -> dict:
    """Atualiza só o canal/som de um push_token já registrado (chamado
    quando a pessoa troca o som nas Configurações do app, depois do
    primeiro login). Se o token ainda não estiver registrado, registra
    agora — cobre o caso de trocar o som antes do primeiro checkin."""
    validos = {"padrao1", "padrao2", "alarme1", "alarme2"}
    if canal not in validos:
        raise ValueError(f"canal inválido — use um de: {', '.join(sorted(validos))}")
    return registrar_push_token_gerenciado(cliente_id, push_token, canal)
...
def atualizar_galpao(cliente_id: str, galpao_id: str, campos: dict) -> dict:
    """Usado tanto pelo admin (renomear nome/identificador) quanto pelo
    cliente (ajustar limite/modo_alerta). Pra renomear o identificador
    (o código que o ESP32 usa pra mandar checkin), passe `novo_id` nos
    campos — é tratado à parte dos demais, que só sobrescrevem valores
    simples."""
        ...
        for galpao in dados[cliente_id]["galpoes"]:
            if galpao["id"] == galpao_id:
                for chave, valor in campos.items():
                    if chave in permitidos:
                        galpao[chave] = valor
                novo_id = campos.get("novo_id")
                if novo_id and novo_id != galpao_id:
                    if any(g["id"] == novo_id for g in dados[cliente_id]["galpoes"]):
                        raise ValueError("já existe um galpão com esse identificador nesse cliente")
                    galpao["id"] = novo_id
                _escrever_atomico(ARQ_GERENCIADOS, dados)
                return dados[cliente_id]
...
def remover_galpao(cliente_id: str, galpao_id: str) -> dict:
    """Remove só esse galpão do cliente — o cliente e o resto dos dados
    dele (outros galpões, contatos, senha) continuam intactos."""
    with _lock:
        dados = _ler(ARQ_GERENCIADOS)
        if cliente_id not in dados:
            raise ValueError("cliente não encontrado")
        galpoes = dados[cliente_id]["galpoes"]
        restantes = [g for g in galpoes if g["id"] != galpao_id]
        if len(restantes) == len(galpoes):
            raise ValueError("galpão não encontrado")
        dados[cliente_id]["galpoes"] = restantes
        _escrever_atomico(ARQ_GERENCIADOS, dados)
        return dados[cliente_id]
...
def registrar_push_token_gerenciado(cliente_id: str, push_token: str, canal: str = "padrao1") -> None:
    """Chame quando o app loga com sucesso (ou quando a pessoa troca o
    som de alerta nas Configurações) — guarda o token + qual canal/som
    esse aparelho específico quer usar. Cada push_token tem seu próprio
    canal — o som escolhido é uma preferência do APARELHO, guardada
    localmente nele (AsyncStorage), não do cliente/perfil (por isso não
    fica sincronizada entre aparelhos diferentes do mesmo cliente)."""
    with _lock:
        dados = _ler(ARQ_GERENCIADOS)
        if cliente_id not in dados:
            raise ValueError("cliente não encontrado")
        tokens = dados[cliente_id].setdefault("push_tokens", [])
        # compatibilidade com registros antigos, salvos como string pura
        tokens[:] = [{"token": t, "canal": "padrao1"} if isinstance(t, str) else t for t in tokens]
        existente = next((t for t in tokens if t["token"] == push_token), None)
        if existente:
            existente["canal"] = canal
        else:
            tokens.append({"token": push_token, "canal": canal})
        _escrever_atomico(ARQ_GERENCIADOS, dados)
...
# ===================== HISTÓRICO DE TEMPERATURA (PERSISTIDO) =====================
#
# Só pra clientes GERENCIADOS — quem decide se chama essa função é
# whatsapp_alertas.marcar_online, checando antes se o galpão pertence a
# um cliente gerenciado (autoatendimento continua só com o histórico em
# memória de 24h, sem persistir, como já era).
#
# Guardamos 1 ponto a cada 20min e mantemos só o último ano — depois
# disso o ponto é descartado sozinho, pra o arquivo não crescer pra
# sempre. Com esse intervalo, 1 ano = ~26.280 pontos por galpão, tamanho
# de arquivo tranquilo pro disco.

RETENCAO_DIAS_HISTORICO = 365
INTERVALO_MIN_PONTO_SEGUNDOS = 20 * 60


def adicionar_ponto_historico(galpao_id: str, valor: float, timestamp: float) -> None:
    with _lock:
        dados = _ler(ARQ_HISTORICO_TEMPERATURA)
        pontos = dados.setdefault(galpao_id, [])
        if pontos and (timestamp - pontos[-1]["timestamp"]) < INTERVALO_MIN_PONTO_SEGUNDOS:
            return  # ainda não passou o intervalo mínimo, não grava de novo
        pontos.append({"valor": valor, "timestamp": timestamp})
        corte = timestamp - RETENCAO_DIAS_HISTORICO * 86400
        dados[galpao_id] = [p for p in pontos if p["timestamp"] >= corte]
        _escrever_atomico(ARQ_HISTORICO_TEMPERATURA, dados)


def historico_persistido_do_galpao(
    galpao_id: str, inicio_ts: float | None = None, fim_ts: float | None = None
) -> list[dict]:
    """Sem `inicio_ts`/`fim_ts`, retorna as últimas 24h (fallback, mantém
    compatibilidade). Com os dois, retorna só os pontos entre eles —
    quem calcula esses limites é o APP, já convertidos pro fuso horário
    do celular da pessoa (o servidor não precisa saber de fuso horário
    nenhum, só filtra entre dois timestamps que já vêm prontos)."""
    with _lock:
        pontos = _ler(ARQ_HISTORICO_TEMPERATURA).get(galpao_id, [])

    if inicio_ts is not None and fim_ts is not None:
        return [p for p in pontos if inicio_ts <= p["timestamp"] < fim_ts]

    corte = time.time() - 24 * 3600
    return [p for p in pontos if p["timestamp"] >= corte]
