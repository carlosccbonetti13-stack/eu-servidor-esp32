...
#
# Guardamos 1 ponto a cada 5min (mesmo intervalo do histórico em
# memória) e mantemos só os últimos RETENCAO_DIAS — depois disso o ponto
# é descartado sozinho, pra o arquivo não crescer pra sempre.

RETENCAO_DIAS_HISTORICO = 30
INTERVALO_MIN_PONTO_SEGUNDOS = 5 * 60
...
def historico_persistido_do_galpao(galpao_id: str, data_str: str | None = None) -> list[dict]:
    """Sem `data_str`, retorna as últimas 24h (comportamento padrão, pra
    manter compatibilidade). Com `data_str` no formato 'YYYY-MM-DD',
    retorna só os pontos daquele dia específico (calculado em UTC, que é
    o fuso que os timestamps já usam)."""
    with _lock:
        pontos = _ler(ARQ_HISTORICO_TEMPERATURA).get(galpao_id, [])

    if data_str:
        try:
            inicio = datetime.strptime(data_str, "%Y-%m-%d").replace(tzinfo=timezone.utc)
        except ValueError:
            return []
        fim = inicio + timedelta(days=1)
        return [p for p in pontos if inicio.timestamp() <= p["timestamp"] < fim.timestamp()]
