...
def _alertar_queda(galpao_id: str):
    ...
    if cliente:
        ...
        # Push pro app: só se a pessoa ligou "notificar ociosidade" nesse
        # galpão específico (cada galpão decide independente, igual no
        # autoatendimento — mesmo que estejam na mesma conta).
...
def marcar_online(galpao_id: str, temperatura: float = None):
    with _lock:
        agora = time.time()
        # DIAG
        print(f"[DIAG-MARCAR] PID={os.getpid()} | id recebido={galpao_id!r} | id do dict antes={id(_ultimo_checkin)}")
        estava_em_queda = _em_queda.get(galpao_id, False)
        _ultimo_checkin[galpao_id] = agora
        # DIAG
        print(f"[DIAG-MARCAR] PID={os.getpid()} | gravado. chaves agora no dict: {list(_ultimo_checkin.keys())!r}")
...
def verificar_quedas():
    ...
    with _lock:
        ...
        if "TESTE01" in _ultimo_checkin:
            gap = agora - _ultimo_checkin["TESTE01"]
            print(f"[DIAG] PID={os.getpid()} | TESTE01: {gap:.0f}s desde o último checkin | em_queda atual: {_em_queda.get('TESTE01', False)}")
        else:
            print(f"[DIAG] PID={os.getpid()} | TESTE01 não está em _ultimo_checkin | id do dict={id(_ultimo_checkin)} | chaves existentes: {list(_ultimo_checkin.keys())!r}")
