...
def marcar_online(galpao_id: str, temperatura: float = None):
    with _lock:
        agora = time.time()
        # DIAG
        print(f"[DIAG-MARCAR] id recebido={galpao_id!r} | id do dict antes={id(_ultimo_checkin)}")
        estava_em_queda = _em_queda.get(galpao_id, False)
        _ultimo_checkin[galpao_id] = agora
        # DIAG
        print(f"[DIAG-MARCAR] gravado. chaves agora no dict: {list(_ultimo_checkin.keys())!r}")
...
def verificar_quedas():
    ...
    with _lock:
        ...
        if "TESTE01" in _ultimo_checkin:
            ...
            print(f"[DIAG] TESTE01: {gap:.0f}s desde o último checkin | em_queda atual: {_em_queda.get('TESTE01', False)}")
        else:
            print(f"[DIAG] TESTE01 não está em _ultimo_checkin | id do dict={id(_ultimo_checkin)} | chaves existentes: {list(_ultimo_checkin.keys())!r}")
