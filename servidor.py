...
def _iniciar_threads_de_fundo():
    """As threads de monitoramento de queda e backup precisam iniciar
    sempre que o módulo é carregado — não só quando rodado direto com
    `python servidor.py`. Isso importa porque em produção o Gunicorn
    IMPORTA este arquivo como módulo (`servidor:app`), em vez de
    executá-lo diretamente; se essas linhas ficassem só dentro do
    `if __name__ == "__main__":` lá embaixo, elas nunca rodariam sob
    Gunicorn — as threads simplesmente não existiriam, sem erro nenhum
    pra avisar."""
...
if __name__ == "__main__":
    ...
    # produção, quem sobe o servidor é o Gunicorn — ver Procfile/comando
    # de start configurado na Render, que não passa por aqui.
