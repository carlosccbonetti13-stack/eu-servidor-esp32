...
def monitorar_quedas():
    while True:
        try:
            ...
            # CRÍTICO: sem esse try/except, qualquer erro aqui dentro
            # (rede, API externa, o que for) mata essa thread pra
            # sempre, silenciosamente — a checagem de queda simplesmente
            # para de rodar, sem travar o servidor e sem erro visível em
            # lugar nenhum, até o próximo deploy reiniciar tudo.
            print(f"[monitorar_quedas] ERRO (thread seguiu rodando): {e}")
        time.sleep(5)


def reenviar_alertas_temperatura():
    """Reenvia push de temperatura fora do limite a cada 5min (ver
    REENVIO_ALERTA_SEGUNDOS em whatsapp_alertas.py) pra quem ainda não
    apertou 'Resolvido'. Checa a cada 60s — não precisa ser mais rápido
    que isso, é só quem decide reenviar de verdade é o próprio intervalo
    guardado no estado."""
    while True:
        try:
            alertas.reenviar_alertas_pendentes()
        except Exception as e:
            print(f"[reenviar_alertas_temperatura] ERRO (thread seguiu rodando): {e}")
        time.sleep(60)
...
def login():
    ...
    if tipo == "cliente":
        ...
        if push_token:
            clientes.registrar_push_token_gerenciado(cliente_id, push_token, corpo.get("canal_som", "padrao1"))
        ...
        return jsonify({
            ...
            "pushover_user_key": cliente.get("pushover_user_key"),
...
@app.route("/admin/cliente/<cliente_id>", methods=["DELETE"])
def admin_remover_cliente(cliente_id):
    erro = _exige_admin()
    if erro:
        return erro
    try:
        clientes.remover_gerenciado(cliente_id)
...
@app.route("/admin/cliente/<cliente_id>/galpao/<galpao_id>", methods=["PUT"])
def admin_editar_galpao(cliente_id, galpao_id):
    erro = _exige_admin()
    if erro:
        return erro
    corpo = request.get_json(force=True) or {}
    try:
        cliente = clientes.atualizar_galpao(cliente_id, galpao_id, corpo)
        return jsonify(cliente), 200
    except ValueError as e:
        return jsonify({"erro": str(e)}), 404


@app.route("/admin/cliente/<cliente_id>/galpao/<galpao_id>", methods=["DELETE"])
def admin_remover_galpao(cliente_id, galpao_id):
    erro = _exige_admin()
    if erro:
        return erro
    try:
        cliente = clientes.remover_galpao(cliente_id, galpao_id)
        return jsonify(cliente), 200
    except ValueError as e:
        return jsonify({"erro": str(e)}), 404
...
@app.route("/admin/cliente/<cliente_id>/pushover", methods=["PUT"])
def admin_ajustar_pushover(cliente_id):
    erro = _exige_admin()
    if erro:
        return erro
    corpo = request.get_json(force=True) or {}
    try:
        cliente = clientes.atualizar_pushover_gerenciado(cliente_id, corpo)
        return jsonify(cliente), 200
    except ValueError as e:
        return jsonify({"erro": str(e)}), 404
...
@app.route("/cliente/<cliente_id>/pushover", methods=["PUT"])
def cliente_ajustar_pushover(cliente_id):
    cliente_id_autenticado = _cliente_autenticado()
    if cliente_id_autenticado != cliente_id:
        return jsonify({"erro": "não autenticado"}), 401
    ...
    corpo = request.get_json(force=True) or {}
    try:
        resultado = clientes.atualizar_pushover_gerenciado(cliente_id, corpo)
        return jsonify(resultado), 200
    except ValueError as e:
        return jsonify({"erro": str(e)}), 404


@app.route("/cliente/<cliente_id>/galpao/<galpao_id>/alerta/resolver", methods=["POST"])
def cliente_resolver_alerta(cliente_id, galpao_id):
    cliente_id_autenticado = _cliente_autenticado()
    if cliente_id_autenticado != cliente_id:
        return jsonify({"erro": "não autenticado"}), 401

    alertas.resolver_alerta_temperatura(galpao_id)
    return jsonify({"ok": True}), 200


@app.route("/cliente/<cliente_id>/push_token", methods=["POST"])
def cliente_atualizar_canal_push_token(cliente_id):
    """Chamado pelo app quando a pessoa troca o som de alerta nas
    Configurações — atualiza (ou registra, se ainda não existir) o canal
    daquele push_token específico. O som escolhido é salvo localmente no
    aparelho (AsyncStorage); isso aqui só avisa o servidor qual canal
    usar quando mandar push pra ESSE aparelho."""
    cliente_id_autenticado = _cliente_autenticado()
    if cliente_id_autenticado != cliente_id:
        return jsonify({"erro": "não autenticado"}), 401
    ...
    corpo = request.get_json(force=True) or {}
    push_token = corpo.get("push_token")
    canal = corpo.get("canal", "padrao1")
    if not push_token:
        return jsonify({"erro": "push_token é obrigatório"}), 400
    try:
        clientes.atualizar_canal_push_token(cliente_id, push_token, canal)
        return jsonify({"ok": True}), 200
    except ValueError as e:
        return jsonify({"erro": str(e)}), 400
...
def app_historico_galpao(galpao_id):
    ...
    # O app já calcula esses limites no fuso horário local do celular —
    # o servidor só filtra entre os dois timestamps, sem se preocupar
    # com fuso horário nenhum.
    inicio_ts = request.args.get("inicio", type=float)
    fim_ts = request.args.get("fim", type=float)
    return jsonify(clientes.historico_persistido_do_galpao(galpao_id, inicio_ts, fim_ts)), 200
...
@app.route("/admin/identificadores", methods=["GET"])
def admin_listar_identificadores():
    """Lista todo identificador de galpão/dispositivo já em uso no
    sistema — tanto de clientes gerenciados quanto de autoatendimento.
    Usa isso antes de gravar um ESP32 novo, pra garantir que o código
    escolhido não bate com um que já existe."""
    erro = _exige_admin()
    if erro:
        return erro
    usados = []
    for cliente_id, cliente in clientes.listar_gerenciados().items():
        for galpao in cliente.get("galpoes", []):
            usados.append({
                ...
                "galpao_nome": galpao.get("nome"),
            })
    for codigo in clientes.listar_autoatendimento().keys():
        usados.append({"id": codigo, "tipo": "autoatendimento", "cliente_id": None, "cliente_nome": None, "galpao_nome": None})
    usados.sort(key=lambda x: x["id"])
    return jsonify(usados), 200
...
def _iniciar_threads_de_fundo():
    """As threads de monitoramento de queda e backup. IMPORTANTE: essa
    função NÃO deve ser chamada automaticamente na importação do módulo
    — descobrimos que isso faz elas nascerem no processo MESTRE do
    Gunicorn (que só administra, nunca atende requisição HTTP nenhuma),
    porque é nesse processo que o Gunicorn importa o app pela primeira
    vez, antes de "clonar" pro processo operário que atende de verdade
    — e threads não sobrevivem a esse clone. O resultado: a checagem
    rodava sozinha, olhando pra uma cópia da memória que nunca recebia
    checkin nenhum.

    Por isso, quem chama essa função agora é o hook `post_fork` do
    Gunicorn (ver gunicorn.conf.py na raiz do projeto) — que roda depois
    do processo operário já existir, garantindo que a thread nasce no
    lugar certo. Em desenvolvimento local (`python servidor.py` direto,
    sem Gunicorn), é chamada no bloco `if __name__ == "__main__"`
    abaixo."""
    threading.Thread(target=monitorar_quedas, daemon=True).start()
    threading.Thread(target=reenviar_alertas_temperatura, daemon=True).start()
    threading.Thread(target=backup_git.loop_backup_diario, daemon=True).start()


if __name__ == "__main__":
    # Só usado em desenvolvimento local (`python servidor.py`). Em
    # produção, quem sobe o servidor é o Gunicorn — ver gunicorn.conf.py,
    # que chama _iniciar_threads_de_fundo() no lugar certo.
    _iniciar_threads_de_fundo()
